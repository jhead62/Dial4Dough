...
angular.module('app').directive('appSignup', ['appConfig',
    function(appConfig) {
        return {
            restrict: 'A',
            scope: {
                    ...
                    if ($scope.signupForm.$valid) {
                        if ($scope.formVals.password_confirm != $scope.formVals.password) {
                            ...
                            if (vals.first_name.length < minNameLength || vals.last_name.length < minNameLength) {
                                $scope.$emit('evtAppalertAlert', {
                                    type: 'error',
                                    msg: 'Must enter a first and last name of at least ' + minNameLength + ' characters each'
                                });
                                valid = false;
                            }
                            ...
                            if (valid) {
                                ...
                                $scope.$emit('evtAppalertAlert', {
                                    type: 'error',
                                    msg: 'Please properly enter all values!'
                                });
                            }
                        }
                    }
